...
/* =========================================================
   ...
   ========================================================= */
document.getElementById('loginInsteadBtn').addEventListener('click', async () => {
  const email = document.getElementById('signupContact').value.trim();
  const password = document.getElementById('signupPassword').value;
  const errorEl = document.getElementById('signupError');
  const btn = document.getElementById('loginInsteadBtn');

  errorEl.textContent = '';

  if (!email || !password) {
    errorEl.textContent = 'Enter your email and password to log in.';
    return;
  }
  ...
  const name = data.user.user_metadata?.name || 'friend';
  document.getElementById('greetingText').textContent = `Welcome back, ${name}`;
  showScreen('screen-home');
});
